...
function isDateInPeriod(date, period) {
  ...
  const startOfRange = new Date(period.start);
  const endOfRange = new Date(period.end);
  if (givenDate >= startOfRange && givenDate <= endOfRange) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  ...
  const formattedDate = `${month}/${day}/${year}, ${hour}:${String(minute).padStart(2, '0')}:${String(second).padStart(2, '0')} ${timeFormat}`;
...
function getWeekNumberByDate(date) {
  ...
  return (
    Math.floor((finalDate - firstWeekStart) / (1000 * 60 * 60 * 24 * 7)) + 1
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (workDayCounter < countWorkDays) {
      ...
      workDayCounter += 1;
    } else if (offDayCounter < countOffDays) {
      offDayCounter += 1;
    }

    if (workDayCounter === countWorkDays && offDayCounter === countOffDays) {
      workDayCounter = 0;
      offDayCounter = 0;
    }
